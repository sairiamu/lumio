...
import {
  ...
  Filter, Plus, Minus, Edit, Trash, LogOut
} from 'lucide-react';
...
export const CardNode: React.FC<NodeProps<Node<NodeData>>> = (props) => {
  ...
  // Keyboard shortcut to close expanded view
  useEffect(() => {
  ...
  }, [isExpanded, id, setExpandedNodeId, updateNodeData]);
  ...
  const handleQuit = useCallback(() => {
    setExpandedNodeId(null);
    updateNodeData(id, { viewMode: 'compact' });
  }, [id, setExpandedNodeId, updateNodeData]);
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span className="font-sora font-bold text-[12px] truncate flex-1 uppercase tracking-wide">
            {data.title || 'Untitled'}
          </span>
          {isExpanded && (
        ...
        <div className="flex-1 p-3 overflow-hidden text-text/90">
          ...
          {(cardMode === 'document' || cardMode === 'mixed') && (
            <div className="text-[11px] line-clamp-3 opacity-90 overflow-hidden">
              {renderLumioMarkdown(data.content || '', true)}
            </div>
          )}
        </div>
  ...
  const renderExpanded = () => {
    return createPortal(
      ...
      >
        <div
          className="bg-canvas border-2 border-accent rounded-2xl shadow-[0_32px_120px_rgba(0,0,0,0.6)] flex flex-col overflow-hidden w-[92%] max-w-[1100px] h-[85%] max-h-[90vh] min-w-[420px] min-h-[350px] animate-in zoom-in-95 duration-400 ease-[cubic-bezier(0.34,1.56,0.64,1)]"
          onClick={e => e.stopPropagation()}
        >
          <div
            className="flex items-center gap-4 px-8 py-5 text-white sticky top-0 z-10 shadow-xl"
            style={{ backgroundColor: accentColor }}
          >
            {cardMode === 'document' ? <FileText size={22} /> : IconComponent && <IconComponent size={24} />}
            <h2 className="font-sora font-black text-2xl flex-1 truncate tracking-tight">{data.title || 'Untitled'}</h2>

            <div className="flex items-center gap-3">
              <button
                onClick={handleQuit}
                className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl transition-all text-sm font-bold border border-white/10 group"
              >
          ...
          <div className="flex-1 overflow-y-auto p-[40px] md:p-[60px] custom-scrollbar bg-canvas/80">
  ...
  const renderEditor = () => {
    ...
    return createPortal(
      <div
        className="fixed inset-0 z-[2000] flex items-center justify-center bg-void/40 backdrop-blur-sm"
        onClick={() => setIsEditing(false)}
      >
        <div
          className="bg-panel border-2 border-accent rounded-xl flex flex-col overflow-hidden shadow-[0_32px_64px_rgba(0,0,0,0.5)] w-[480px] h-[650px] animate-in fade-in zoom-in-95 duration-200"
          onClick={e => e.stopPropagation()}
          onKeyDown={e => {
        ...
        >
          {/* Editor Header */}
          <div className="flex items-center justify-between px-6 py-4 bg-void/60 border-b border-border">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-accent/20 rounded-lg text-accent">
                <Edit size={18} />
              </div>
              <div>
                <h3 className="text-sm font-black uppercase tracking-widest text-text">Edit Card Content</h3>
                <p className="text-[10px] text-text-muted font-medium">Customize modes, icons, and documents</p>
              </div>
            </div>
            <button
              onClick={() => setIsEditing(false)}
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-5 custom-scrollbar">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[10px] uppercase font-black text-text-muted flex justify-between">
                Title
                <span className="opacity-50 font-normal lowercase italic">Ctrl+Enter to save</span>
              </label>
              <input
                autoFocus
                className="bg-void/50 border border-border rounded-lg px-3 py-2.5 text-sm outline-none focus:border-accent transition-colors shadow-inner"
            ...
            {(localData.cardMode === 'simple' || localData.cardMode === 'mixed') && (
              <div className="flex flex-col gap-2">
                <label className="text-[10px] uppercase font-black text-text-muted flex justify-between items-center">
                  Icon Selector
                  <div className="relative w-40">
                    <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
                    <input
                      className="w-full bg-void/50 border border-border rounded-full pl-8 pr-3 py-1.5 text-[10px] outline-none focus:border-accent shadow-inner"
                ...
                <div className="grid grid-cols-6 gap-2 h-36 overflow-y-auto bg-void/30 p-3 rounded-xl border border-border custom-scrollbar shadow-inner">
                    ...
                    return (
                      <button
                        ...
                        className={`p-2.5 rounded-lg transition-all flex items-center justify-center ${
                          localData.cardIcon === name ? 'bg-accent text-white shadow-lg shadow-accent/20 scale-110' : 'hover:bg-white/10 text-text-muted hover:text-text'
                        }`}
                        title={name}
                      >
                        <Icon size={18} />
            ...
            {(localData.cardMode === 'document' || localData.cardMode === 'mixed') && (
              <div className="flex flex-col gap-2 flex-1 min-h-[180px]">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] uppercase font-black text-text-muted">Lumio Markdown</label>
                  <button
                    onClick={handleImportFile}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-accent/10 hover:bg-accent/20 text-[10px] font-black text-accent rounded-lg transition-colors border border-accent/20"
                  >
                    <FileUp size={14} /> IMPORT DOC
                  </button>
                </div>
                <textarea
                  className="flex-1 bg-void/50 border border-border rounded-xl p-4 text-[12px] font-mono outline-none focus:border-accent resize-none custom-scrollbar leading-relaxed shadow-inner"
            ...
            {(localData.cardMode === 'simple' || !localData.cardMode) && (
              <div className="flex flex-col gap-4">
                <label className="text-[10px] uppercase font-black text-text-muted">Parameters</label>
                <div className="flex flex-col gap-2">
                  {(localData.parameters || []).map((p, i) => (
                    <div key={i} className="flex gap-2 items-center group">
                      <input
                        className="w-1/3 bg-void/50 border border-border rounded-lg px-3 py-2 text-[11px] outline-none focus:border-accent shadow-inner"
                        value={p.key}
                        onChange={e => {
                      ...
                      />
                      <input
                        className="flex-1 bg-void/50 border border-border rounded-lg px-3 py-2 text-[11px] outline-none focus:border-accent shadow-inner"
                        value={p.value}
                        onChange={e => {
                          const newParams = [...(localData.parameters || [])];
                      ...
                      <button
                        ...
                        className="p-1.5 text-text-muted hover:text-error transition-colors opacity-0 group-hover:opacity-100"
                      >
                        <X size={16} />
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="text-[10px] font-black text-accent flex items-center gap-2 hover:opacity-80 transition-opacity mt-2 ml-1"
                  >
                ...
                <div className="flex flex-col gap-2 mt-2">
                  ...
                  <textarea
                    className="bg-void/50 border border-border rounded-xl p-4 text-[11px] outline-none focus:border-accent resize-none h-24 shadow-inner"
          ...
          {/* Footer */}
          <div className="p-4 border-t border-border flex justify-end gap-3 bg-void/50 backdrop-blur-md">
            <button
              onClick={() => setIsEditing(false)}
              className="px-5 py-2 text-[11px] font-bold hover:bg-white/5 rounded-xl transition-colors text-text-muted hover:text-text"
            >
              Cancel
            </button>
            <button
              onClick={handleCloseEdit}
              className="px-6 py-2 bg-accent text-white text-[11px] font-black rounded-xl shadow-lg shadow-accent/20 flex items-center gap-2 hover:brightness-110 transition-all active:scale-95"
            >
